        ...
        <div class="row">
            ...
            @if(!empty($posts))
                @foreach($posts as $post)
                    <!-- $posts คือ ตัวแปรที่ีมีค่า 1 ค่า หรือเก็บค่ามากกว่า 1 ค่า อาจจะเป็น Array หรือตัวแปรชนิดอื่นๆที่มีการเก็บค่าหลายๆค่าไว้
                    $post เฉยๆ ไม่มี s คือ ตัวแปรที่ทำหน้าที่วิ่งส่ง วิ่งใช้ทีละ 1 ค่า จาก $posts วนไปเรื่อยๆตามจำนวนที่มีอยู่ใน $posts ดังนั้นถ้าไม่มี s คือตัวแปรที่ใช้วิ่งวน คล้ายกับตัวแปร i ใน for ของภาษา C เช่น
  for(i=0;i++;i<$posts)
  for(i=1;i++;i<=$posts)//มี 2 ตัว
  for(i=1;i++;i<$posts+1)//มี 2 ตัว
  {cout<<i;}

  //วนย้อนหลัง
  for(i=$posts;i--;i<0)
  for(i=2;i--;2<0)
    //ค่าเริ่มต้น; การเปลี่ยนแปลงค่า(update); เงื่อนไข
        i=2;            i--;             2<0
    Grammar ของการเบียนโค้ด
  for(i=2;i--;1<0)//มี 2 ตัว
  for(i=2;i--;1<0)//มี 2 ตัว
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="mr-2">
                                            <a href="{{ url('/profile/'.$post->user->id) }}">
                                                <img src="{{$post->user->image ? $post->user->image:'http://picsum.photos/50/50'}}" width="45" class="rounded-circle">
                                            </a>
                                        </div>
                                            ...
                                            @foreach($post->comments as $comment)
                                                <li class="media p-2 mt-2">
                                                    <a href="{{url('/profile/'.$comment->user->id)}}">
                                                        <img src="{{ $comment->user->image ? $comment->user->image:'http://picsum.photos/50/50' }}" class="rounded-circle mr-3" width="30">
                                                    </a>
